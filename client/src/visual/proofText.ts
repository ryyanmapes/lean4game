...
/** One display entry per player interaction. Multi-line Lean generated by a
 * single gesture stays grouped under one step number in the proof sidebar. */
export function displayedProofSteps(steps: ProofTextStep[], mode: 'lean' | 'play'): string[] {
  return steps.map(step => [
    step.rotation,
    shortenQualifiedNames(leafForMode(step, mode)),
  ].filter(Boolean).join('\n'))
}
